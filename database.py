...
def get_or_create_user(user_id, username="O'yinchi", ref_by=None):
    ...
    with get_connection() as conn:
        ...
        if not user:
            ...
            # Referal bonusini qo'shish (+1000 coin)
            if ref_by and str(ref_by) != str(user_id):
                cursor.execute('SELECT * FROM users WHERE user_id = ?', (ref_by,))
                if cursor.fetchone():
        ...
        # Energiya tiklanish mantiqi (har 3 soniyada +1 energiya)
        ...
        if energy_to_add > 0 and user['energy'] < user['max_energy']:
            ...
            # Agar vaqt o'tmagan bo'lsa ham so'nggi vaqtni yangilamasdan saqlaymiz
            pass
...
def process_tap(user_id):
    ...
    with get_connection() as conn:
        ...
        # Avval energiyani yangilab olamiz
...
def update_score_and_energy(user_id, score_change):
    ...
    with get_connection() as conn:
        ...
        row = cursor.fetchone()
        if row:
            new_score = max(0, row['score'] + score_change)
            cursor.execute('UPDATE users SET score = ?, last_energy_update = ? WHERE user_id = ?', (new_score, current_time, user_id))
            conn.commit()
