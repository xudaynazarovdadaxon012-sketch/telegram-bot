...
# ADMIN ID RAXAMI (O'zingizning Telegram ID'ingizni kiriting)
ADMIN_ID = 8898979946

bot = Bot(token=BOT_TOKEN)
dp = Dispatcher()

# FOYDANUVCHILAR BAZASI (Xotirada saqlash)
users_db = set()
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        for user_id in users_db:
            try:
                ...
                pass
        await message.answer(f"✅ Xabar **{count} ta** foydalanuvchiga muvaffaqiyatli yuborildi!", parse_mode="Markdown")
...
async def start_translator(message: types.Message, state: FSMContext):
    ...
    await message.answer("🔤 **Matn Tarjimon:**\n\nTarjima qilmoqchi bo'lgan matningizni yuboring (Avtomatik o'zbek tiliga o'giriladi):", parse_mode="Markdown")

@dp.message(UserStates.waiting_for_translate)
async def process_translation(message: types.Message, state: FSMContext):
    if message.text in ["🎮 Mini App (O'yinlar Hub)", "🤖 Sun'iy Intellekt (AI)", "🎨 AI Rasm Yaratish", "📥 Video Yuklagich", "📈 Kripto & Oltin", "🔗 Link Qisqartirish", "📲 QR-Kod Yaratish", "🧮 Aqlli Kalkulyator", "🌤 Aniq Ob-havo", "💎 Valyuta kurslari", "📝 Shaxsiy Eslatmalar", "⚙️ Admin Panel"]:
        await state.clear()
...
async def process_qr(message: types.Message, state: FSMContext):
    if message.text in ["🎮 Mini App (O'yinlar Hub)", "🤖 Sun'iy Intellekt (AI)", "🎨 AI Rasm Yaratish", "📥 Video Yuklagich", "📈 Kripto & Oltin", "🔗 Link Qisqartirish", "abc Matn Tarjimon", "🧮 Aqlli Kalkulyator", "🌤 Aniq Ob-havo", "💎 Valyuta kurslari", "📝 Shaxsiy Eslatmalar", "⚙️ Admin Panel"]:
        await state.clear()
        await handle_text_messages(message, state)
        return
...
async def get_currency(message: types.Message):
    try:
        ...
        msg = (
            f"💎 **Markaziy Bank rasmiy valyuta kurslari:**\n\n"
            f"🇺🇸 1 USD = **{usd} UZS**\n"
            f"🇪🇺 1 EUR = **{eur} UZS**\n"
            f"🇷🇺 1 RUB = **{rub} UZS**"
        )
